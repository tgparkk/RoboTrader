#!/usr/bin/env python3
"""
병합 데이터셋으로 ML 모델 학습 (기존 시스템 보존)

입력: ml_dataset_merged.csv
출력: ml_model_merged.pkl (새 파일, 기존 ml_model.pkl 및 ml_model_dynamic_pl.pkl 보존)
"""
...
# 설정 (기존 파일과 충돌 없음)
INPUT_FILE = 'ml_dataset_merged.csv'
OUTPUT_MODEL = 'ml_model_merged.pkl'
OUTPUT_REPORT = 'ml_training_report_merged.txt'
...
def train_lightgbm_model(X_train, y_train, X_val, y_val, feature_names):
    ...
    model = lgb.train(
        params,
        lgb_train,
        num_boost_round=1000,  # 데이터 많아서 증가
...
def save_model(model, feature_names, label_encoder, auc_score):
    ...
    model_data = {
        ...
        'version': f'merged_{pd.Timestamp.now().strftime("%Y%m%d")}',
...
def main():
    ...
    print("=" * 70)
    print(f"\n생성된 파일:")
    print(f"  - 모델: {OUTPUT_MODEL}")
    print(f"  - 리포트: {OUTPUT_REPORT}")
    print(f"\n기존 파일 보존:")
    print(f"  ✅ ml_model.pkl (기존 모델)")
    print(f"  ✅ ml_model_dynamic_pl.pkl (동적 손익비 모델)")
    print(f"  ✅ ml_dataset.csv (기존 데이터)")
    print(f"  ✅ ml_dataset_dynamic_pl.csv (동적 손익비 데이터)")
