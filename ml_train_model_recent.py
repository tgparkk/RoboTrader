#!/usr/bin/env python3
"""
최신 2개월(11-12월) 데이터로 ML 모델 학습

입력: ml_dataset_recent.csv
...
def load_and_prepare_data(csv_file: str = 'ml_dataset_recent.csv'):
    """데이터 로드 및 전처리"""
    print("=" * 70)
    print("📂 데이터 로드 중...")
    print("=" * 70)
...
def plot_feature_importance(model, feature_names, top_n=20):
    ...
    plt.title(f'Top {top_n} Feature Importance (11-12월 모델)')
    plt.tight_layout()
    plt.savefig('ml_feature_importance_recent.png', dpi=150)
    print("\n✅ 특징 중요도 그래프 저장: ml_feature_importance_recent.png")
...
def plot_roc_curve(y_test, y_pred_proba):
    ...
    plt.title('ROC Curve (11-12월 모델)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('ml_roc_curve_recent.png', dpi=150)
    print("✅ ROC 곡선 저장: ml_roc_curve_recent.png")
...
def save_model(model, feature_names, label_encoder, auc_score, filename='ml_model.pkl'):
    ...
    model_data = {
        ...
        'data_source': 'ml_dataset_recent.csv',
...
def main():
    ...
    print("=" * 70)
    print("\n생성된 파일:")
    print("  - ml_model.pkl (11-12월 학습 모델)")
    print("  - ml_feature_importance_recent.png")
    print("  - ml_roc_curve_recent.png")
