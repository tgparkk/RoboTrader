...
🤖 ML 필터 기능:
  - 기존 기술적 분석 신호에 ML 예측 결과 추가
  - 승률이 낮은 신호는 자동 차단
  - ML 예측 결과를 로그에 상세 표시

사용 예 (Windows PowerShell):
  # candidate_stocks 테이블에서 자동으로 종목 조회 (ML 필터 적용)
  python utils\signal_replay_ml.py --date 20250825 --export txt --charts
  
  # 특정 종목 직접 지정 (ML 필터 적용)
  python utils\signal_replay_ml.py --date 20250808 \
...
- 🤖 ML 예측기로 매수 신호의 승률/수익률 예측
- ML 필터 통과한 신호만 최종 매수 신호로 처리
- ML 차단된 신호는 별도 표시

주의:
- ML 모델이 학습되어 있어야 합니다 (trade_analysis/ml_models/*.pkl)
...
def initialize_ml_predictor():
    """ML 예측기 초기화 (전역 변수)"""
    global ml_predictor, ml_predictor_available
    ...
    try:
        ...
        if model_dir.exists() and any(model_dir.glob("*.pkl")):
            ...
            print("✅ ML 예측기 초기화 완료 - ML 필터가 적용됩니다")
        else:
            print("⚠️ ML 모델 파일이 없습니다 - 기본 신호만 사용됩니다")
            ml_predictor_available = False
            
    except Exception as e:
        print(f"⚠️ ML 예측기 초기화 실패: {e} - 기본 신호만 사용됩니다")
        ml_predictor_available = False

def apply_ml_filter(signal_strength, stock_code: str, target_date: str):
    """
    ML 필터 적용
    
    Returns:
        Tuple[bool, str, dict]: (통과여부, 사유, ML예측결과)
    """
    if not ml_predictor_available or not ml_predictor:
        return True, "ML 필터 비활성화", {}
    ...
    try:
        # ML 예측 실행
        ml_result = ml_predictor.predict_trade_outcome(stock_code, target_date, "pullback_pattern")
        
        if "error" in ml_result:
            return True, "ML 예측 오류", {}
...
def list_all_buy_signals(df_3min: pd.DataFrame, *, logger: Optional[logging.Logger] = None, stock_code: str = "UNKNOWN", target_date: str = "20250901") -> List[Dict[str, object]]:
    ...
    try:
        ...
        for i in range(len(df_3min)):
            ...
            # 🤖 ML 필터 적용
            ml_pass, ml_reason, ml_result = apply_ml_filter(signal_strength, stock_code, target_date)
            
            # ML 차단된 신호는 건너뜀 (로그에 기록은 남김)
            if not ml_pass:
                logger.debug(f"🚫 {stock_code} {time_str} ML 차단: {ml_reason}")
                continue
...
def simulate_trades(df_3min: pd.DataFrame, df_1min: Optional[pd.DataFrame] = None, *, logger: Optional[logging.Logger] = None, stock_code: str = "UNKNOWN", target_date: str = "20250901") -> List[Dict[str, object]]:
    ...
    try:
        # 매수 신호 리스트 가져오기
        buy_signals = list_all_buy_signals(df_3min, logger=logger, stock_code=stock_code, target_date=target_date)
...
def main():
    print("🤖 ML 필터 적용된 신호 재현 시스템")
    print("=" * 60)
    
    # ML 예측기 초기화
    initialize_ml_predictor()
    
    parser = argparse.ArgumentParser(description="🤖 ML 필터가 적용된 눌림목(3분) 매수신호 재현 리포트")
    ...
    # 병렬 처리를 위한 함수 정의
    def process_single_stock(stock_code: str) -> Tuple[str, List[Dict[str, object]], pd.DataFrame, List[Dict[str, object]]]:
        """단일 종목 처리 함수"""
        try:
                        ...
                        try:
                            ...
                            if all_data_for_stock is not None and not all_data_for_stock.empty:
                                ...
                                if df_3min_detailed is not None and not df_3min_detailed.empty:
                                    ...
                                    buy_signals_for_mapping = list_all_buy_signals(df_3min_detailed, logger=logger, stock_code=stock_code, target_date=date_str)
                                    ...
                                    for i, row in df_3min_detailed.iterrows():
                                        ...
                                        if len(current_data) >= 5:
                                            ...
                                            # 🤖 ML 필터 적용
                                            original_signal_strength = signal_strength  # 원본 신호 보존
                                            ml_pass = True
                                            ml_reason = "ML 필터 비활성화"
                                            
                                            if signal_strength:
                                                ml_pass, ml_reason, ml_result = apply_ml_filter(signal_strength, stock_code, date_str)
                                                if not ml_pass:
                                                    signal_strength = None  # ML 차단 시 신호 무효화
                                            ...
                                            if signal_strength:
                                                ...
                                                if ml_predictor_available:
                                                    status_parts.append(f"🤖{ml_reason}")
                                            else:
                                                # ML 차단된 경우 원본 신호와 차단 사유 표시
                                                if original_signal_strength:
                                                    status_parts.append(f"🚫ML차단({original_signal_strength.signal_type.value})")
                                                    status_parts.append(f"차단사유:{ml_reason}")
                                                else:
