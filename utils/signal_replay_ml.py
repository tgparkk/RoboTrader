...
def extract_features_from_pattern(pattern_info: dict) -> dict:
    ...
    try:
        ...
        # 회복률
        recovery_rate = 0
        if support_candles_list and breakout.get('candle'):
            support_min_price = min(c.get('low', float('inf')) for c in support_candles_list)
            breakout_close = breakout.get('candle', {}).get('close', 0)
        ...
        features = {
            ...
            # 패턴 품질 피처 (신규 6개)
            'breakout_vol_vs_prev_candle': breakout_vol_vs_prev_candle,
            'support_low_consistency': support_low_consistency,
            'uptrend_consecutive_bullish': uptrend_consecutive_bullish,
            'decline_bearish_ratio': decline_bearish_ratio,
            'support_vol_below_quarter': support_vol_below_quarter,
            'breakout_body_vs_support_avg': breakout_body_vs_support_avg,
        }
