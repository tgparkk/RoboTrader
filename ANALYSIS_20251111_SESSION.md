...
### 4. filter_stats.py (새 파일) + pullback_candle_pattern.py + utils/signal_replay.py ✅
**목적**: 필터 통계를 자동으로 수집하고 리플레이 로그에 기록
...
**출력 위치**:
1. `signal_new2_replay_YYYYMMDD_9_00_0.txt` 파일 상단 (당일 수익 요약 다음)
2. `batch_signal_replay.py` 실행 후 화면 출력
...
전체 패턴 체크: 50건
  ✅ 통과: 30건 (60.0%)
  🚫 마이너스 조합 필터 차단: 8건 (16.0%)
  🚫 종가 위치 필터 차단: 12건 (24.0%)
```
