#!/usr/bin/env python3
"""
12월-1월 데이터(36개 특징)로 ML 모델 학습

입력: ml_dataset_dec_jan.csv (12-1월, 36개 특징)
출력: ml_model.pkl (기존 덮어쓰기)
"""
...
def load_and_prepare_data(csv_file: str = 'ml_dataset_dec_jan.csv'):
    """데이터 로드 및 전처리"""
    print("=" * 70)
    print("📂 데이터 로드 중...")
    print("=" * 70)
...
def train_lightgbm_model(X_train, y_train, X_val, y_val, feature_names):
    """LightGBM 모델 학습"""
    print("\n" + "=" * 70)
    print("🚀 LightGBM 모델 학습 시작 (36개 특징, 12월-1월 데이터)")
    print("=" * 70)
...
def plot_feature_importance(model, feature_names, top_n=20):
    ...
    for i, row in feature_importance_df.head(top_n).iterrows():
        ...
        print(f"{is_new} {row['feature']:35s}: {row['importance']:8.0f}")

    return feature_importance_df


def save_model(model, feature_names, label_encoder, auc_score, filename='ml_model.pkl'):
    """모델 저장"""
    model_data = {
        'model': model,
        ...
        'version': 'enhanced_36features_dec_jan',
        'trained_at': pd.Timestamp.now().isoformat(),
        'data_source': 'ml_dataset_dec_jan.csv',
        'data_period': '2025-12-01 ~ 2026-01-09',
        'n_features': len(feature_names),
        'feature_enhancement': 'Added 10 features: volume_std(3), bullish_ratio, depth, recovery, body_ratio, speed(2), length, concentration'
    }
...
def main():
    print("\n" + "=" * 70)
    print("🤖 12월-1월 확장 특징(36개) ML 모델 학습")
    print("=" * 70)

    # 1. 데이터 로드
    X, y, feature_names, label_encoder = load_and_prepare_data()

    # 2. 시간 기반 분할 (60% train, 20% val, 20% test)
    ...
    # 7. 모델 저장
    save_model(model, feature_names, label_encoder, auc, filename='ml_model.pkl')

    print("\n" + "=" * 70)
    print("✅ 모델 학습 및 평가 완료!")
    print("=" * 70)
    print("\n생성된 파일:")
    print("  - ml_model.pkl (36개 특징, 12월-1월 학습 모델)")
    ...
    print(f"2. 특징 수: {len(feature_names)}개 (기존 26개 → 36개)")
    print(f"3. 교차검증: {np.mean(cv_scores):.4f} (± {np.std(cv_scores):.4f})")
    print(f"4. 학습 데이터: 12월-1월 ({len(X)}건)")
    ...
    if auc >= 0.60:
        ...
        print("\n❌ 모델 성능이 낮습니다.")
        print("   → 추가 특징 또는 데이터 개선 필요")

    print("\n다음 단계:")
    print("  python batch_signal_replay_ml.py -s 20260102 -e 20260109")
